## This script will Uninstall and Install NEW TEAMS
...
if (Get-Process | Where-Object {$_.HasExited}) {Write-Output "Force Closed New Teams"} else {Write-Output "New Teams is not open"}

$classicteams = get-process -name "teams"
Stop-Process -InputObject $classicteams -Force
if (Get-Process | Where-Object {$_.HasExited}) {Write-Output "Force Closed Classic Teams"} else {Write-Output "Classic Teams is not open"}

$outlook = get-process -name "Outlook"
Stop-Process -InputObject $outlook -Force
if (Get-Process | Where-Object {$_.HasExited}) {Write-Output "Force Closed Outlook"} else {Write-Output "Outlook is not open"}
...
## Software Uninstallation Completed


Write-Output "Starting New Teams Install do not close the command window that pops up" | Red

## Starting Software Installation
Start-Process "C:\temp\NewTeams_24091.214.2846.1452\teamsbootstrapper.exe" -Args "-p" -wait
## Software Installation Completed

EXIT}

## Associated resource file "NewTeams_24091.214.2846.1452.zip"
## Author = Seth Burns
