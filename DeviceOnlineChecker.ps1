...
<#
.SYNOPSIS
    Device Status Monitor – GUI layout rework.

.DESCRIPTION
    * Replaces manual XY positioning with a **bottom‑docked Panel** that always stays
      visible, even on high‑DPI and resizable windows.
    * DataGridView now docks **Fill** so it grows/shrinks with the form.
    * Minimum form size enforced so controls never collapse.
    * All previous features retained (configurable interval, toasts, sound cue,
      multi‑ping confirmation, parallel pings, group column).
...
$form                       = [System.Windows.Forms.Form]@{
    ...
    MinimumSize     = [Drawing.Size]::new(760,560)
...
$lblIP    = [System.Windows.Forms.Label]@{Text='IP/Host:';AutoSize=$true;Location=[Drawing.Point]::new(220,5)}
$txtIP    = [System.Windows.Forms.TextBox]@{Location=[Drawing.Point]::new(285,2);Size=[Drawing.Size]::new(150,22)}

$lblGroup = [System.Windows.Forms.Label]@{Text='Group:';AutoSize=$true;Location=[Drawing.Point]::new(455,5)}
$txtGroup = [System.Windows.Forms.TextBox]@{Location=[Drawing.Point]::new(505,2);Size=[Drawing.Size]::new(120,22)}

$btnAdd   = [System.Windows.Forms.Button]@{Text='Add';Location=[Drawing.Point]::new(640,0);Size=[Drawing.Size]::new(80,26)}
...
$numInterval.Location = [Drawing.Point]::new(300,45)
...
function Invoke-PingBatch {
    ...
    $jobs = foreach ($row in $deviceRows) {
        $addr=$row.Cells[1].Value
        $ps=[powershell]::Create(); $ps.RunspacePool=$pool
        $null=$ps.AddScript("Test-Connection -ComputerName '$addr' -Count 1 -Quiet -TimeoutSeconds 2")
        [pscustomobject]@{Row=$row;PS=$ps;Handle=$ps.BeginInvoke()}
    }
    foreach ($j in $jobs) {
        $online=[bool]($j.PS.EndInvoke($j.Handle)); $j.PS.Dispose()
        $row=$j.Row; $name=$row.Cells[0].Value
        $device=$script:devices | Where-Object Name -eq $name; if (-not $device){continue}

        if ($online) { $device.ConsecutiveFailures=0; $status='Online' }
        else { $device.ConsecutiveFailures++; $status=($device.ConsecutiveFailures -ge $PingCount)?'Offline':'Online' }
